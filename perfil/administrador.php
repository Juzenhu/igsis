	...
	<section id="listarUser" class="home-section bg-white">
	 <div class="form-group">
		 ...
		 <h2>Usuários Cadastrados</h2>
				<a href="?perfil=administrador&p=users" class="btn btn-theme btn-lg btn-block">Inserir novo usuário</a>
  	        </div>
				</div> 
		<div class="container">
      			  <div class="row">
...
case "novoUser": // INSERIR NOVO USUARIO 
	if(isset($_POST['inserirUser'])){
		...
		if($existe['numero'] == 0){
			$sql_inserir = "INSERT INTO `ig_usuario` (`idUsuario`, `ig_papelusuario_idPapelUsuario`, `senha`, `receberNotificacao`, `nomeUsuario`, `email`, `nomeCompleto`, `idInstituicao`, `telefone`) VALUES (NULL, '$perfil', '$senha', '$receberEmail', '$usuario', '$email', '$nomeCompleto', '$instituicao', '$telefone')";
			$query_inserir = mysqli_query($sql_inserir);
			if($query_inserir){
			...
			<div class="col-md-offset-1 col-md-10">  
				...
				<div class="col-md-offset-2 col-md-8">
                	...
                	<div class="col-md-offset-2 col-md-8">
						...
						<input type="checkbox" name="mostrarSenha" id="senha"/><label style="padding:0 10px 0 5px;"> Exibir Senha</label>
               		</div> <!-- // Departamento !-->
					<div class="col-md-offset-2 col-md-8">	
					...
					<div class="col-md-offset-2 col-md-8">
						...
						<?php instituicaoLocal("ig_instituicao","3",""); ?>
						</select>
                	</div>  <!-- // Perfil de Usuario !-->
					 <div class="col-md-offset-2 col-md-8">
					 <div class="col-md-offset-2 col-md-8">
...
if(isset($_POST['cadastrar'])){
		...
		$espaco = $_POST['espaco'];	
			?><form method='POST' action='?perfil=administrador&p=espacos'><?php
			if($espaco == ''){  
			$mensagem = "<p>O campo espaco, está em branco e é obrigatório. <a href='?perfil=administrador&p=espacos'>Tente novamente.</a></p>"; 
							 } 
		else { $existe = mysqli_num_rows(mysqli_query("SELECT * FROM ig_espaco WHERE espaco = '$espaco' "));
		$espaco = $_POST['espaco'];	
		if($existe == 0) { // caso não esteja vazio
			
			$sql_inserir = "INSERT INTO `ig_espaco` (`idEspaco`,`espaco`,`publicado`) VALUES (NULL, '$espaco', 1)";
			...
			$mensagem = "Espaço já existente. Utilize outro espaço.";
		}
	...
	<section id="list_items" class="home-section bg-white">
		<div class="container">
				  ...
				  <div class="col-md-offset-2 col-md-8">
					...
					<h4>Selecione o evento para recuperar ou edita-ló.</h4>
                    <h5><?php if(isset($mensagem)){echo $mensagem;} ?></h5>
					</div>
				  </div>
			  </div>  
